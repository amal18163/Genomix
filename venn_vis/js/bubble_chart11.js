d3.csv("jamie.csv", function(data1) {
  d3.csv("jessie.csv", function(data2) {

function bubbleChart() {
  // Constants for sizing

  var width = window.innerWidth * 0.9 * (7 / 12) -50; //956.56; //(window.innerWidth) * 0.9 * (7 / 12) - 50; //956.56; //1200
  var height = window.innerHeight * 0.8; //860; //(window.innerHeight); //860
  variantNodes = [];
  ...
  var columnCenters = {
    ...
    2: { x: width / 2, y: height / 2.1 },
    3: { x: 2 * width / 3, y: height / 2.1 }
  };
 ...
 /*Determines which column of the venn diagram a variant belongs to
  ...
  /*This data manipulation function takes the raw data from both CSV files
    ...
    -----------------------------------------------------------------*/
      ...
      console.log("shared variants: " + commonVariants.length);
...
-----------------------------------------------------------------------*/
  ...
  /*Function prepares the data for visualization by adding an svg element
    ...
    svg = d3.select(selector)
      ...
      .attr('width', width)
      .attr('height', height)
      .attr('background', 'url(../img/venn.png)');

    // svg.append('rect')
    //    .attr("width", "956.56")
    //    .attr("height", "860")
    //    .attr("background:", "url(../img/venn.png) no-repeat");
   ...
   createPattern1('PathDiag', 'img/carrier_path.png');
   createPattern1('BenDiag', 'img/carrier_benign.png');
   createPattern1('ProtDiag', 'img/carrier_prot.png');
   createPattern1('ProtDiag', 'img/carrier_pharm.png');
  //  createPattern1('PharmDiag', "2.4");
    ...
    bubbles.enter().append('circle')
      ...
      .attr('fill', function (d) {
          ...
          //  console.log("d.color: " + d.color);
           return d.color; })
      .attr('stroke', function (d) { return "#aaa"; }) //d3.rgb(bubbleStrokes[d.impact]).darker(); })
      .attr('stroke-width', 2)
...
-----------------------------------------------------------------------*/
  ...
  function onBubble(d) {
      if (d3.select(this).attr("class") != "bubble clicked" || d3.select(this).attr("opacity") == 1) {
  ...
  function offBubble(d) {
    console.log("d.clicked: " + d3.select(this).attr("class"));
      if (d3.select(this).attr("class") != "bubble clicked") {
          d3.select(this).attr('stroke', '#aaa') //d3.rgb(bubbleStrokes[d.impact]).darker())
                     .attr('stroke-width', '2')
                     .attr('cursor', 'none');
      }
  }
  ...
  function show_details(d) {
    ...
    d3.select(this).attr('stroke', '#EABE3C')
                   .attr('stroke-width', '4')
                    .attr('cursor', 'default');
    ...
    document.getElementById("var-category-info").innerHTML=categories_string;
    document.getElementById("var-inheritance-info").innerHTML=d.inheritance;
    if (d.inheritance == "Recessive" && d.zygosity == "Heterozygous"){
      document.getElementById("var-affected-info").innerHTML='Carrier'; //'0'; //carrier
    } else {
      document.getElementById("var-affected-info").innerHTML='Affected'; //'1'; //affected
    }
  }
  ...
  function hideDetail(d) {
    // reset outline
    d3.select(this)
      .attr('stroke', '#aaa') //d3.rgb(d.color).darker());
